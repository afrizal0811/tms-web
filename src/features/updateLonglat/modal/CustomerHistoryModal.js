...
export default function CustomerHistoryModal({ isOpen, onClose, data, customerName, dateRange }) {
  ...
  return (
    <div
      className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200 cursor-pointer"
      style={{ zIndex: 9999 }}
      onClick={onClose}
    >
      <div
        className="bg-white rounded-xl shadow-2xl w-full max-w-7xl flex flex-col h-[90vh] cursor-default"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex justify-between items-start p-5 border-b border-gray-100 bg-gray-50 rounded-t-xl shrink-0">
          <div>
        ...
        {/* Content Body */}
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden rounded-b-xl">
          {/* Kolom Kiri: Tabel */}
          <div className="w-full lg:w-1/2 flex-1 overflow-auto border-r border-gray-200 flex flex-col order-1">
            {data && data.length > 0 ? (
          ...
          {/* Kolom Kanan: Peta */}
          <div className="w-full lg:w-1/2 bg-slate-50 h-[400px] lg:h-full relative border-t lg:border-t-0 lg:border-l border-gray-200 shrink-0 order-2">
            {mapData.length > 0 ? (
              <div className="absolute inset-0 p-4">
                {/* UpdateMap akan di-mount ulang karena Parent (Modal) di-mount ulang.
                   Otomatis menjalankan useEffect FitBounds (tampilan awal).
                */}
                <UpdateMap
                  data={mapData}
