...
const hasAnyValue = (data) =>
  Object.values(data).some((section) => {
    ...
    if (section && typeof section === 'object') {
      return Object.values(section).some((v) => v !== null && v !== 0);
    }
...
export default function DashboardDetailTab({ loading, summaryData }) {
  ...
  const isHasData = hasAnyValue(summaryData);

  return (
    <div className="space-y-10 animate-in fade-in duration-300 h-full flex flex-col flex-1">
      {!isHasData ? (
        // UBAH 2: Tambahkan 'min-h-[400px]' untuk menjaga konsistensi visual
        <div className="flex-1 flex items-center justify-center bg-gray-50 border border-dashed border-gray-300 rounded-xl text-gray-400 min-h-[400px]">
