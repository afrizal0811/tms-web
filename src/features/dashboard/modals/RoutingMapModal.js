...
// UPDATE: ArrowPolyline sekarang menerima segments (bukan 1 array posisi panjang)
// agar bisa mewarnai per-segment (Hijau saat dipilih)
  ...
  return (
    <>
      {segments.map((seg, i) => {
        ...
        // Posisi Ujung (90% perjalanan, agar tidak tertutup marker bulat tujuan)
        const endLat = start[0] + (end[0] - start[0]) * 0.9;
        const endLng = start[1] + (end[1] - start[1]) * 0.9;

        return (
          // FIX: Tambahkan 'color' atau 'isHighlight' ke key agar React me-render ulang saat berubah
  ...
  return (
    <div className="absolute top-12 right-4 z-900 w-72 bg-white rounded-lg shadow-xl border border-gray-200 overflow-hidden animate-in fade-in slide-in-from-right-4 duration-300">
      <div className="bg-slate-50 p-3 border-b border-gray-100 flex justify-between items-start">
        <div>
          <h3 className="font-bold text-sm text-slate-800 leading-tight">{cleanName}</h3>
...
const MapViewSection = ({
  ...
  const pathSegments = useMemo(() => {
    ...
    // --- 1. SETUP POINTS (Sama seperti sebelumnya) ---
    if (!isActualMap) {
      // RO: Loop Full
      points.push({ coords: hubCoords, name: 'HUB' });
      routeTasks.forEach((t) => {
        const name = parseCustomerString(t.customerName).name;
        points.push({ coords: [t.lat, t.lng], name: name });
      });
    ...
    // --- 2. BUILD SEGMENTS WITH HIGHLIGHT ---
    for (let i = 0; i < points.length - 1; i++) {
      ...
      // Logic Highlight:
      // Garis Hijau adalah garis yang KELUAR dari titik yang dipilih.
      // Jika pilih "HUB", garis HUB -> Point 1 jadi hijau.
      // Jika pilih "Cust A", garis Cust A -> Point B jadi hijau.

      if (selectedCustomer) {
        if (selectedCustomer === 'HUB') {
          // Jika user pilih HUB, highlight segmen pertama (index 0)
          // ATAU segmen yang start-nya bernama HUB
          if (currentPoint.name === 'HUB') isHighlight = true;
        } else {
          // Jika user pilih Customer lain
    ...
    return segments;
  }, [sortedTasks, isActualMap, selectedCustomer]);
  ...
  return (
    <div className="flex flex-col h-1/2 bg-white border rounded-lg overflow-hidden shadow-sm relative">
      ...
      {showInfoCard && activeTask && <InfoCard task={activeTask} onClose={onCloseCard} />}
...
export default function RoutingMapModal({ isOpen, onClose, data }) {
  ...
  const customerOptions = useMemo(() => {
    ...
    sortedTasks.forEach((t) => {
      ...
      const cleanName = parseCustomerString(t.customerName).name;
      if (cleanName && !uniqueMap.has(cleanName)) {
        uniqueMap.set(cleanName, cleanName);
      }
    });

    const hasHub = driverTasks.some((t) => t.type === 'HUB_START');
    let options = Array.from(uniqueMap.values());
    if (hasHub) options = ['HUB', ...options];

    return options;
  }, [driverTasks]);

  const handleFocusCustomer = (customerName) => {
    setSelectedCustomer(customerName);

    if (!customerName) return;

    const targetTask = driverTasks.find((t) => {
      if (customerName === 'HUB' && t.type === 'HUB_START') return true;
      const name = parseCustomerString(t.customerName).name;
  ...
  const onMarkerClick = (task) => {
    ...
    if (task && task.customerName) {
      const cleanName = parseCustomerString(task.customerName).name;
      handleFocusCustomer(cleanName);
    }
  };
