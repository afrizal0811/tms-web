...
export async function getVehicleMappings(hubId = null) {
  ...
  return await apiFetch(
    `/api/mileapp/vehicle-mappings${queryString}`,
...
export async function updateVehicleMapping(id, plat, mappedType) {
  ...
  let res = await fetch('/api/mileapp/vehicle-mappings', { method: 'PUT', ...options });
  if (!res.ok) {
    res = await fetch('/api/mileapp/vehicle-mappings', { method: 'PATCH', ...options });
  }
  if (!res.ok) {
